  ...
  Disposable
{
  ...
  init(
    id: Any?, plugin: InAppWebViewFlutterPlugin?, frame: CGRect,
  ...
  public func prepareAndAddUserScripts() {
    ...
    if javaScriptBridgeEnabled {
      ...
      configuration.userContentController.addPluginScript(
        ConsoleLogJS.CONSOLE_LOG_JS_PLUGIN_SCRIPT(allowedOriginRules: pluginScriptsOriginAllowList))
      configuration.userContentController.addPluginScript(
      ...
      configuration.userContentController.addPluginScript(
        OriginalViewPortMetaTagContentJS.ORIGINAL_VIEWPORT_METATAG_CONTENT_JS_PLUGIN_SCRIPT(
          allowedOriginRules: pluginScriptsOriginAllowList))
      configuration.userContentController.addPluginScript(
        OnScrollChangedJS.ON_SCROLL_CHANGED_EVENT_JS_PLUGIN_SCRIPT(
          allowedOriginRules: pluginScriptsOriginAllowList))
      if let settings = settings {
    ...
    if let settings = settings {
      ...
      if settings.allowUniversalAccessFromFileURLs {
        configuration.setValue(
          settings.allowUniversalAccessFromFileURLs, forKey: "allowUniversalAccessFromFileURLs")
      }
          ...
          let cgImage = screenshot.cgImage(forProposedRect: nil, context: nil, hints: nil)
        {
          ...
          if let with = with {
            ...
            case "JPEG":
              ...
              imageData = newRep.representation(
                using: .jpeg,
                properties: [
  ...
  public func loadUrl(urlRequest: URLRequest, allowingReadAccessTo: URL?) {
    ...
    if let allowingReadAccessTo = allowingReadAccessTo, url.scheme == "file",
      allowingReadAccessTo.scheme == "file"
    {
  ...
  public func loadData(
    ...
    if let allowingReadAccessTo = allowingReadAccessTo, baseUrl.scheme == "file",
      allowingReadAccessTo.scheme == "file"
    {
  ...
  func setSettings(newSettings: InAppWebViewSettings, newSettingsMap: [String: Any]) {
      ...
      if let applePayAPIEnabled = settings?.applePayAPIEnabled, !applePayAPIEnabled {
        if javaScriptBridgeEnabled {
          enablePluginScriptAtRuntime(
            flagVariable: OnLoadResourceJS.FLAG_VARIABLE_FOR_ON_LOAD_RESOURCE_JS_SOURCE(),
      ...
      if let applePayAPIEnabled = settings?.applePayAPIEnabled, !applePayAPIEnabled {
        if javaScriptBridgeEnabled {
          enablePluginScriptAtRuntime(
            ...
            pluginScript: InterceptAjaxRequestJS.INTERCEPT_AJAX_REQUEST_JS_PLUGIN_SCRIPT(
              allowedOriginRules: newSettings.pluginScriptsOriginAllowList,
              forMainFrameOnly: newSettings.pluginScriptsForMainFrameOnly))
        }
      } else {
    ...
    if newSettingsMap["interceptOnlyAsyncAjaxRequests"] != nil
      && settings?.interceptOnlyAsyncAjaxRequests != newSettings.interceptOnlyAsyncAjaxRequests
    {
      if let applePayAPIEnabled = settings?.applePayAPIEnabled, !applePayAPIEnabled,
    ...
    if newSettingsMap["useShouldInterceptFetchRequest"] != nil
      && settings?.useShouldInterceptFetchRequest != newSettings.useShouldInterceptFetchRequest
    {
      if let applePayAPIEnabled = settings?.applePayAPIEnabled, !applePayAPIEnabled {
        if javaScriptBridgeEnabled {
          enablePluginScriptAtRuntime(
    ...
    if newSettingsMap["mediaPlaybackRequiresUserGesture"] != nil
      && settings?.mediaPlaybackRequiresUserGesture != newSettings.mediaPlaybackRequiresUserGesture
    {
      if #available(macOS 10.12, *) {
        configuration.mediaTypesRequiringUserActionForPlayback =
    ...
    if newSettingsMap["suppressesIncrementalRendering"] != nil
      && settings?.suppressesIncrementalRendering != newSettings.suppressesIncrementalRendering
    {
    ...
    if newSettingsMap["allowUniversalAccessFromFileURLs"] != nil
      && settings?.allowUniversalAccessFromFileURLs != newSettings.allowUniversalAccessFromFileURLs
    {
      configuration.setValue(
        newSettings.allowUniversalAccessFromFileURLs, forKey: "allowUniversalAccessFromFileURLs")
    }
      ...
      let runCallback = {
        if let channelDelegate = self.channelDelegate {
          channelDelegate.onReceivedHttpAuthRequest(
            challenge: HttpAuthenticationChallenge(fromChallenge: challenge), callback: callback)
        } else {
          callback.defaultBehaviour(nil)
        }
      }
      ...
      let runCallback = {
        if let channelDelegate = self.channelDelegate {
          channelDelegate.onReceivedServerTrustAuthRequest(
            challenge: ServerTrustChallenge(fromChallenge: challenge), callback: callback)
        } else {
          callback.defaultBehaviour(nil)
        }
      }
  ...
  public func webView(
    ...
    if let channelDelegate = channelDelegate {
      channelDelegate.onJsAlert(
        url: frame.request.url, message: message, isMainFrame: frame.isMainFrame, callback: callback
      )
    } else {
      callback.defaultBehaviour(nil)
    }
  ...
  func createPromptDialog(
    ...
    completionHandler(
      value != nil ? value : (res == .alertFirstButtonReturn ? txt.stringValue : nil))
  }
  ...
  public func webView(
    ...
    let windowWebView = InAppWebView(
      id: nil, plugin: nil, frame: CGRect.zero, configuration: configuration)
  ...
  public func userContentController(
    ...
    guard let bridgeSecret = body["_bridgeSecret"] as? String, bridgeSecret == exceptedBridgeSecret
    else {
    //   print(
    ...
    if let javaScriptHandlersOriginAllowList = settings?.javaScriptHandlersOriginAllowList {
      if let origin = sourceOrigin?.absoluteString {
        for allowedOrigin in javaScriptHandlersOriginAllowList {
          if origin.range(of: allowedOrigin, options: .regularExpression, range: nil, locale: nil)
            != nil
          {
        ...
        if let args = body["args"] as? String, let data = args.data(using: .utf8) {
          ...
          if let jsonData = jsonArgs?.first, let jsObjectName = jsonData["jsObjectName"] as? String
          {
              ...
              where: ({ ($0.jsObjectName == jsObjectName) }))
            {
              ...
              if !scheme.isEmpty, !host.isEmpty {
                sourceOrigin = URL(
                  string: "\(scheme)://\(host)\(port != 0 ? ":" + String(port) : "")")
              }
  ...
  public func scrollTo(x: Int, y: Int, animated: Bool) {
    evaluateJavaScript(
      "window.scrollTo({left: \(x), top: \(y), behavior: \(animated ? "'smooth'" : "'auto'")})")
  }

  public func scrollBy(x: Int, y: Int, animated: Bool) {
    evaluateJavaScript(
      "window.scrollBy({left: \(x), top: \(y), behavior: \(animated ? "'smooth'" : "'auto'")})")
  }
  ...
  public func printCurrentPage(
    ...
    completionHandler: PrintJobController.CompletionHandler? = nil
  ) -> String? {
    if #available(macOS 11.0, *) {
      ...
      if let settings = settings {
        ...
        if let time = settings.time {
          printInfoDictionary[.time] = Date(
            timeIntervalSince1970: TimeInterval(Double(time) / 1000))
        }
      }
      ...
      if let settings = settings {
        if let pageOrder = settings.pageOrder,
          let order = NSPrintOperation.PageOrder.init(rawValue: pageOrder)
        {
      ...
      if let id = printJobId, let plugin = plugin {
        let printJob = PrintJobController(
          plugin: plugin, id: id, job: printOperation, settings: settings)
        ...
        printOperation.runModal(
          for: window, delegate: self, didRun: #selector(printOperationDidRun), contextInfo: nil)
      } else {
        printView(self)
      }
